...
void TVPDumpOSContext(const CONTEXT &ctx)
{
	...
	TJS_snprintf(buf, BUF_SIZE, TJS_W("Index Registers   : ESI:0x%08X  EDI:0x%08X"),
		ctx.Esi, ctx.Edi);
#elif defined _M_ARM
	TJS_snprintf(buf, BUF_SIZE, TJS_W("Index Registers   : R0:0x%08X  R1:0x%08X  R2:0x%08X"),
...
#elif defined _M_ARM
	TJS_snprintf(buf, BUF_SIZE, TJS_W("Pointer Registers : R11:0x%08X  SP:0x%08X  PC:0x%08X"),
		ctx.R11, ctx.Sp, ctx.Pc);
#endif
	TVPAddLog(buf);
#if defined _M_IX86 || defined _M_AMD64
	// - Flag Register
